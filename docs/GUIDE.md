...
|---|---|---|
| **profile README** (`diogo-costa-silva`) | first thing on your profile — hero + **featured 4-6** + links | `build:profile` |
| **`portfolio` README** | the pinned repo / "All projects" target — **featured + everything by theme** | `build:readme` |
| **live website** (`.github.io`) | the rich interactive vitrine | `build:v1` → deploy chain |
...
| `npm run build:readme` | Rebuild **this repo's** README showcase (featured + all by theme) |
| `npm run build:profile` | Print the **profile** featured table (stdout) |
...
```bash
npm run build:readme    # rebuilds README.md between the PORTFOLIO markers
```
...
npm run build:readme                          # → this repo's GitHub vitrine
